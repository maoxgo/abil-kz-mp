<template>
<uni-shadow-root class="wxml-to-canvas-old-index"><canvas v-if="use2dCanvas" id="weui-canvas" type="2d" :style="'width: '+(width)+'px; height: '+(height)+'px;'"></canvas>
...
global['__wxRoute'] = 'wxml-to-canvas-old/index'
        ...
        wx.downloadFile({
          ...
          success(res) {
            if (res.statusCode === 200) {
              ...
              //   content: res.message
              // })
              reject(new Error(`downloadFile:fail1 ${res.message}`))
          ...
          fail(res) {
            ...
            //   content: e.message
            // })
            reject(new Error(`downloadFile:fail2 ${JSON.stringify(res)}; ${img}`))
            // reject(new Error(`downloadFile:fail ${img}`))
          }
        })
      } else {
...
export default global['__wxComponents']['wxml-to-canvas-old/index']
</script>
