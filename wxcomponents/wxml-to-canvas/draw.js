class Draw {
  ...
  drawText(text, box, style) {
    ...
    if (typeof lineHeight === 'string') { // 2em
      lineHeight = Math.ceil(parseFloat(lineHeight.replace('em')))
    }
    ...
    ctx.font = `${fontWeight} ${fontSize}px ${fontFamily || 'sans'}`
    ...
    console.log(text)
    const textReverse = this.reverseArabicChars(text)
    console.log(textReverse)
    ctx.fillText(textReverse, x, y + inlinePaddingTop)
    ctx.restore()
    return
    ...
    if (textWidth <= w) {
      ...
      // console.log(text)
      // console.log(chars)
      // this.fillChars(chars, x-ctx.measureText(text).width/2, y+inlinePaddingTop)
      return
    }
  ...
  reverseArabicChars(text) {
    const reg = /([\u0600-\u06ff]|[\u0750-\u077f]|[\ufb50-\ufc3f]|[\ufe70-\ufefc]|\s)+/g
